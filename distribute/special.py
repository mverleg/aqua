...
def special_1(request):
	...
	return HttpResponse('hello world %d / %d' % (len(TimeSlot.objects.filter(roster__name = normalroster)), len(TimeSlot.objects.filter(roster__name = normalroster))))
...
def special_2(request):
	...
	return HttpResponse('bye world %d / %d' % (len(TimeSlot.objects.filter(roster__name = normalroster)), len(TimeSlot.objects.filter(roster__name = normalroster))))
