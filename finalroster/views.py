...
def assignment_submit(request, assignment):
    ...
    if 'action' in request.POST.keys():
        if request.POST['action'] == 'keep':
            ...
            users = [worker.user for worker in RosterWorker.objects.exclude(pk = request.user.pk).order_by('user__first_name')]
...
def assignment_submit_staff_empty(request, timeslot):
    ...
    try:
        ...
        return notification(request, 'Sorry, je kan geen shift toewijzen als je zelf een shift op dat moment hebt. Dit omdat het achter de schermen werkt door tijdelijk jou een shift te geven en die over te zetten. Geef dus tijdelijk even je eigen shift af (of geef hem meteen aan de betreffende persoon en claim dan de lege).', next_page = reverse('assignment_submit_staff_empty', kwargs = { 'timeslot': timeslot.pk }))
    users = [worker.user for worker in RosterWorker.objects.all()]
    return render(request, 'gift_select_user.html', {
        'assignment': assignment,
        'slot': timeslot,
